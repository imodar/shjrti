-- تفعيل إضافة vector للـ embeddings
CREATE EXTENSION IF NOT EXISTS vector;

-- إضافة جداول للبحث الذكي والاقتراحات
CREATE TABLE IF NOT EXISTS public.search_embeddings (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  family_member_id UUID NOT NULL REFERENCES family_tree_members(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS public.smart_suggestions (
  ...
  family_member_id UUID REFERENCES family_tree_members(id) ON DELETE CASCADE,
...
USING (
  EXISTS (
    ...
    WHERE ftm.id = search_embeddings.family_member_id 
    AND ftm.family_id IN (SELECT family_id FROM public.get_user_family_ids(auth.uid()))
  )
);
