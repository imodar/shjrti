...
import { ShoppingCart, TreePine, Package, CreditCard, Check, ArrowRight, Frame, Ruler, Palette, MapPin, Phone, Crown, ChevronRight, ChevronLeft, User, Users, ShoppingBag, Star, Store as StoreIcon } from "lucide-react";
  ...
  return <div dir="rtl" className="min-h-screen bg-gradient-to-br from-background via-accent/5 to-secondary/10 relative overflow-hidden">
      ...
      {/* Main Content */}
        ...
        <div className="max-w-7xl mx-auto px-6 py-16">
                      ...
                      <div className="relative z-10 w-full max-w-md transform group-hover:scale-105 transition-transform duration-500">
                        ...
                        {selectedDesign === 'classic' && <div className="text-center space-y-6">
                            ...
                            <div className="flex justify-center gap-4 mb-4">
                              {mockTreeData.parents.map((parent, i) => <div key={i} className="flex flex-col items-center">
                            ...
                            <div className="flex justify-center gap-2 flex-wrap">
                              {mockTreeData.children.map((child, i) => <div key={i} className="flex flex-col items-center">
                        ...
                        {selectedDesign === 'modern' && <div className="text-center space-y-6">
                            ...
                            <div className="flex flex-col items-center space-y-4">
                              ...
                              <div className="flex gap-2">
                                {mockTreeData.children.map((child, i) => <div key={i} className="w-12 h-12 bg-gradient-to-br from-secondary via-primary to-accent rounded-2xl flex items-center justify-center text-white text-xs font-bold shadow-2xl transform hover:rotate-3 transition-transform">
                                    {child.name.charAt(0)}
                                  </div>)}
                              </div>
                            </div>
              ...
              <Card className="bg-gradient-to-br from-card via-card/95 to-primary/2 backdrop-blur-xl border border-primary/20 shadow-xl overflow-hidden rounded-2xl">
                ...
                <CardHeader>
                  ...
                  return <div key={index} className="flex items-center">
                          <div className={`flex items-center justify-center w-12 h-12 rounded-full border-2 transition-all duration-300 ${index === currentStep ? 'bg-primary border-primary text-white shadow-lg scale-110' : index < currentStep ? 'bg-green-500 border-green-500 text-white' : 'bg-background border-muted-foreground/30 text-muted-foreground'}`}>
                            {index < currentStep ? <Check className="h-5 w-5" /> : <Icon className="h-5 w-5" />}
                          </div>
                          <span className={`mr-2 text-sm font-medium ${index === currentStep ? 'text-primary' : index < currentStep ? 'text-green-600' : 'text-muted-foreground'}`}>
                            {step.title}
                          </span>
                          {index < wizardSteps.length - 1 && <ChevronRight className="h-4 w-4 text-muted-foreground mx-2" />}
                        </div>;
                })}
                  </div>
                </CardHeader>
                ...
                <CardContent className="p-8 border-t border-border/50">
                  ...
                  {currentStep === 0 && <div className="space-y-6">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        {designTemplates.map(template => <div key={template.id} onClick={() => setSelectedDesign(template.id)} className={`relative cursor-pointer group transition-all duration-500 transform ${selectedDesign === template.id ? 'scale-105 z-10' : 'hover:scale-102 hover:-translate-y-1'}`}>
                            {/* Selection Glow Effect */}
                            {selectedDesign === template.id && <div className="absolute -inset-2 bg-gradient-to-r from-primary/30 via-accent/30 to-secondary/30 rounded-2xl blur-xl opacity-75 animate-pulse" />}
                            
                            {/* Main Card */}
                            <div className={`relative overflow-hidden rounded-2xl border-2 transition-all duration-500 backdrop-blur-sm ${selectedDesign === template.id ? 'border-primary/60 bg-gradient-to-br from-primary/15 via-accent/10 to-secondary/5 shadow-2xl' : 'border-border/40 bg-gradient-to-br from-card/90 via-card/70 to-accent/5 hover:border-primary/40 hover:shadow-xl'}`}>
                              
                              {/* Premium Badge Ribbon */}
                              {template.isPremium && <div className="absolute -top-1 -right-1 z-20">
                              ...
                              <div className="p-6 pb-4">
                                 <div className="flex items-center justify-center mb-4">
                                   <div className={`relative w-16 h-16 rounded-2xl flex items-center justify-center transition-all duration-500 ${selectedDesign === template.id ? 'bg-gradient-to-br from-primary/20 via-accent/20 to-secondary/20 shadow-lg transform rotate-3' : 'bg-gradient-to-br from-muted/50 to-accent/10 group-hover:shadow-md group-hover:-rotate-1'}`}>
                                    {/* Background Pattern */}
                                    <div className="absolute inset-0 rounded-2xl opacity-20">
                                     ...
                                     {/* Template Icon */}
                                     <span className={`relative text-3xl transition-all duration-500 ${selectedDesign === template.id ? 'animate-bounce' : 'group-hover:scale-110'}`}>
                                       {template.image}
                                     </span>
                                    
                                    {/* Selection Indicator */}
                                    {selectedDesign === template.id && <div className="absolute -top-2 -right-2 w-8 h-8 bg-primary rounded-full flex items-center justify-center shadow-lg animate-pulse">
                                 ...
                                 {/* Template Name */}
                                 <h3 className={`text-center text-base font-bold mb-2 transition-colors duration-300 ${selectedDesign === template.id ? 'text-primary' : 'text-foreground group-hover:text-primary'}`}>
                                   {template.name}
                                 </h3>
                                 ...
                                 <div className="space-y-1 mb-3">
                                   ...
                                   {template.isPremium && <div className="flex items-center justify-center gap-2 text-xs text-accent font-medium">
                                       <div className="w-1 h-1 bg-current rounded-full"></div>
                              ...
                              <div className={`px-6 py-4 border-t transition-all duration-300 ${selectedDesign === template.id ? 'border-primary/20 bg-gradient-to-r from-primary/5 to-accent/5' : 'border-border/20 bg-gradient-to-r from-muted/5 to-accent/5'}`}>
                                <div className="flex items-center justify-between">
                                  <div className="flex items-center gap-2">
                                    <div className={`px-3 py-1 rounded-full text-sm font-bold transition-all duration-300 ${template.price > 0 ? selectedDesign === template.id ? 'bg-accent/20 text-accent' : 'bg-muted/50 text-muted-foreground group-hover:bg-accent/10' : selectedDesign === template.id ? 'bg-green-100 text-green-700' : 'bg-green-50 text-green-600 group-hover:bg-green-100'}`}>
                                      {template.price > 0 ? `+${template.price} ريال` : 'مجاني'}
                                    </div>
                                  </div>
                                  
                                  {/* Selection Radio */}
                                  <div className={`relative w-5 h-5 rounded-full border-2 transition-all duration-300 ${selectedDesign === template.id ? 'border-primary bg-primary shadow-lg' : 'border-muted-foreground/30 group-hover:border-primary/50'}`}>
                                    {selectedDesign === template.id && <div className="absolute inset-1 bg-white rounded-full animate-scale-in" />}
                                  </div>
                                </div>
                              </div>
                  ...
                  {currentStep === 1 && <div className="space-y-6">
                      ...
                      <RadioGroup value={selectedFrame} onValueChange={setSelectedFrame} className="space-y-3">
                        {frameOptions.map(frame => <div key={frame.id} className={`group relative overflow-hidden rounded-2xl transition-all duration-500 cursor-pointer ${selectedFrame === frame.id ? 'ring-2 ring-accent ring-offset-2 ring-offset-background shadow-2xl shadow-accent/25' : 'hover:shadow-xl hover:-translate-y-1'}`} onClick={() => setSelectedFrame(frame.id)}>
                            {/* Background Gradient */}
                            <div className={`absolute inset-0 transition-all duration-500 ${selectedFrame === frame.id ? 'bg-gradient-to-r from-accent/20 via-secondary/10 to-primary/15' : 'bg-gradient-to-r from-card via-card/95 to-card/90 group-hover:from-accent/5 group-hover:to-secondary/5'}`} />
                            
                            {/* Animated Border */}
                            <div className={`absolute inset-0 rounded-2xl transition-all duration-500 ${selectedFrame === frame.id ? 'border-2 border-accent' : 'border border-border group-hover:border-accent/30'}`} />
                            ...
                            <div className="relative p-6">
                              <div className="flex items-center justify-between">
                                ...
                                <div className="flex items-center gap-4">
                                  <RadioGroupItem value={frame.id} id={frame.id} className={`transition-colors duration-300 ${selectedFrame === frame.id ? 'border-accent text-accent' : 'border-muted-foreground'}`} />
                                  
                                  {/* Frame Icon */}
                                  <div className={`w-10 h-10 rounded-lg flex items-center justify-center transition-all duration-300 ${selectedFrame === frame.id ? 'bg-accent shadow-lg' : 'bg-muted group-hover:bg-accent/20'}`}>
                                    <Frame className={`h-5 w-5 transition-colors duration-300 ${selectedFrame === frame.id ? 'text-white' : 'text-muted-foreground group-hover:text-accent'}`} />
                                  </div>
                                  
                                  <Label htmlFor={frame.id} className={`text-lg font-semibold cursor-pointer transition-colors duration-300 ${selectedFrame === frame.id ? 'text-accent' : 'text-foreground group-hover:text-accent'}`}>
                                    {frame.name}
                                  </Label>
                                </div>
                                ...
                                <div className="flex items-center gap-3">
                                  <span className={`text-xl font-bold transition-colors duration-300 ${selectedFrame === frame.id ? 'text-accent' : 'text-muted-foreground group-hover:text-foreground'}`}>
                                    {frame.price > 0 ? `+${frame.price} ريال` : 'مجاني'}
                                  </span>
                                  
                                  {selectedFrame === frame.id && <div className="w-8 h-8 bg-accent rounded-full flex items-center justify-center animate-scale-in shadow-lg">
                            ...
                            {/* Glow Effect */}
                            {selectedFrame === frame.id && <div className="absolute -inset-1 bg-gradient-to-r from-accent via-secondary to-primary rounded-2xl opacity-20 blur-lg animate-pulse" />}
                  ...
                  {currentStep === 2 && <div className="space-y-6">
                      ...
                      <RadioGroup value={selectedSize} onValueChange={setSelectedSize} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        {sizeOptions.map(size => <div key={size.id} className={`group relative overflow-hidden rounded-xl transition-all duration-500 cursor-pointer ${selectedSize === size.id ? 'ring-2 ring-accent ring-offset-2 ring-offset-background shadow-xl shadow-accent/25' : 'hover:shadow-lg hover:-translate-y-0.5'}`} onClick={() => setSelectedSize(size.id)}>
                            {/* Background Gradient */}
                            <div className={`absolute inset-0 transition-all duration-500 ${selectedSize === size.id ? 'bg-gradient-to-r from-accent/15 via-orange-200/10 to-amber-100/15' : 'bg-gradient-to-r from-card via-card/95 to-card/90 group-hover:from-accent/5 group-hover:to-amber-50/10'}`} />
                            
                            {/* Animated Border */}
                            <div className={`absolute inset-0 rounded-xl transition-all duration-500 ${selectedSize === size.id ? 'border-2 border-accent' : 'border border-border group-hover:border-accent/40'}`} />
                            ...
                            <div className="relative p-4">
                              <div className="flex items-center justify-between">
                                ...
                                <div className="flex items-center gap-3">
                                  <RadioGroupItem value={size.id} id={size.id} className={`transition-colors duration-300 ${selectedSize === size.id ? 'border-accent text-accent' : 'border-muted-foreground'}`} />
                                  
                                  <div className="flex items-center gap-2">
                                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center transition-all duration-300 ${selectedSize === size.id ? 'bg-gradient-to-br from-accent to-amber-400 shadow-md' : 'bg-muted group-hover:bg-accent/20'}`}>
                                      <Ruler className={`h-4 w-4 transition-colors duration-300 ${selectedSize === size.id ? 'text-white' : 'text-muted-foreground group-hover:text-primary'}`} />
                                    </div>
                                    
                                    <Label htmlFor={size.id} className={`font-semibold cursor-pointer transition-colors duration-300 ${selectedSize === size.id ? 'text-accent' : 'text-foreground group-hover:text-accent'}`}>
                                      {size.name}
                                    </Label>
                                  </div>
                                </div>
                                ...
                                {/* Right side - Price and Check */}
                                <div className="flex items-center gap-3">
                                  <span className={`font-bold transition-colors duration-300 ${selectedSize === size.id ? 'text-amber-600' : 'text-muted-foreground group-hover:text-foreground'}`}>
                                    {size.price} ريال
                                  </span>
                                  
                                  {selectedSize === size.id && <div className="w-6 h-6 bg-gradient-to-br from-accent to-amber-500 rounded-full flex items-center justify-center animate-scale-in shadow-md">
                                      <Check className="h-4 w-4 text-white" />
                                    </div>}
                                </div>
                              </div>
                            ...
                            {/* Glow Effect */}
                            {selectedSize === size.id && <div className="absolute -inset-0.5 bg-gradient-to-r from-accent via-amber-400 to-accent rounded-xl opacity-25 blur-md animate-pulse" />}
                      ...
                      {/* Custom Size Options */}
                      {selectedSize === 'custom' && <div className="mt-6 space-y-4">
                          ...
                          <RadioGroup value={selectedCustomSize} onValueChange={setSelectedCustomSize} className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            {customSizeOptions.map(customSize => <div key={customSize.id} className={`group relative overflow-hidden rounded-lg transition-all duration-300 cursor-pointer ${selectedCustomSize === customSize.id ? 'ring-2 ring-accent ring-offset-1 ring-offset-background shadow-lg shadow-accent/20' : 'hover:shadow-md hover:-translate-y-0.5'}`} onClick={() => setSelectedCustomSize(customSize.id)}>
                                {/* Background */}
                                <div className={`absolute inset-0 transition-all duration-300 ${selectedCustomSize === customSize.id ? 'bg-gradient-to-r from-accent/10 via-primary/5 to-secondary/8' : 'bg-gradient-to-r from-card to-card/95 group-hover:from-accent/3 group-hover:to-primary/3'}`} />
                                
                                {/* Border */}
                                <div className={`absolute inset-0 rounded-lg transition-all duration-300 ${selectedCustomSize === customSize.id ? 'border-2 border-accent' : 'border border-border group-hover:border-accent/30'}`} />
                                
                                {/* Content */}
                                <div className="relative p-3">
                                  <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-2">
                                      <RadioGroupItem value={customSize.id} id={customSize.id} className={`transition-colors duration-300 ${selectedCustomSize === customSize.id ? 'border-accent text-accent' : 'border-muted-foreground'}`} />
                                      <Label htmlFor={customSize.id} className={`font-medium cursor-pointer transition-colors duration-300 ${selectedCustomSize === customSize.id ? 'text-accent' : 'text-foreground group-hover:text-accent'}`}>
                                        {customSize.name}
                                      </Label>
                                    </div>
                                    
                                    <div className="flex items-center gap-2">
                                      <span className={`font-bold transition-colors duration-300 ${selectedCustomSize === customSize.id ? 'text-accent' : 'text-muted-foreground group-hover:text-foreground'}`}>
                                        {customSize.price} ريال
                                      </span>
                                      
                                      {selectedCustomSize === customSize.id && <div className="w-5 h-5 bg-accent rounded-full flex items-center justify-center animate-scale-in">
                                ...
                                {/* Glow Effect */}
                                {selectedCustomSize === customSize.id && <div className="absolute -inset-0.5 bg-gradient-to-r from-accent to-primary rounded-lg opacity-15 blur-sm" />}
                              </div>)}
                          </RadioGroup>
                        </div>}
                    </div>}
                  ...
                  <div className="flex justify-between items-center mt-8 pt-6 border-t border-border">
                    <Button variant="outline" onClick={handlePrevStep} disabled={currentStep === 0} className="flex items-center gap-2">
                    ...
                    <Button onClick={handleNextStep} className="flex items-center gap-2 bg-gradient-to-r from-primary to-accent hover:from-primary/90 hover:to-accent/90">
          ...
          {isCustomizationFinished && <div className="mt-16 animate-fade-in">
                ...
                <Card className="bg-gradient-to-br from-card via-card/95 to-primary/2 backdrop-blur-xl border border-primary/20 shadow-xl overflow-hidden rounded-2xl transform-style-3d">
                  <CardHeader className="bg-gradient-to-r from-primary/5 to-accent/5 border-b border-primary/10">
                    <div className="flex items-center justify-between">
                      ...
                      <Button variant="outline" onClick={handleBackToCustomization} className="flex items-center gap-2">
                        <ChevronRight className="h-4 w-4" />
                        تعديل التخصيص
                      </Button>
                    </div>
