...
import { Users, Plus, Search, TreePine, ArrowLeft, Edit, Trash2, Heart, UserPlus, Star, Skull, MoreVertical, Crown, Clock, Store, User, Bell, Baby, FileText, X } from "lucide-react";
...
const FamilyBuilder = () => {
  ...
  // Generation calculation utilities - DRY principle
                ...
                {packageData && familyMembers.length >= packageData.max_family_members ? (
                  <Card className="bg-gradient-to-br from-red-50 to-red-100 dark:from-red-950/50 dark:to-red-900/50 border-2 border-red-200 dark:border-red-800 rounded-2xl">
                    <CardContent className="p-6 flex flex-col items-center justify-center h-full min-h-[200px]">
                      ...
                      <h3 className="font-bold text-red-600 dark:text-red-400 text-lg mb-2">{t('family_builder.max_limit_reached', 'تم الوصول للحد الأقصى')}</h3>
                      <p className="text-red-500 dark:text-red-300 text-center text-sm mb-4">
                        {t('family_builder.max_limit_desc', 'لقد وصلت إلى الحد الأقصى المسموح')} ({packageData.max_family_members} {t('family_builder.members_count', 'أعضاء')})
                      </p>
                      <Button 
