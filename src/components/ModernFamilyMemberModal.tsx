...
export const ModernFamilyMemberModal = ({
  ...
  const populateEditForm = () => {
    ...
    if (editMember.relatedPersonId) {
      ...
      const parentMarriage = marriages.find(m => m.husband?.id === editMember.fatherId && m.wife?.id === editMember.motherId || m.husband?.id === editMember.motherId && m.wife?.id === editMember.fatherId);
      if (parentMarriage) {
        selectedParent = parentMarriage.id;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const sanitizedData = sanitizeFormData({
        ...
        fatherId,
        // Set father ID from selected marriage
        motherId,
  ...
  return <>
      ...
      <div className="fixed inset-0 z-[9998] bg-black/50 flex items-center justify-center p-4">
        ...
        <div className="font-arabic bg-gradient-to-br from-gray-50 via-green-50 to-emerald-50 dark:from-gray-950 dark:via-green-950 dark:to-emerald-950 rounded-2xl shadow-2xl max-w-full sm:max-w-lg md:max-w-4xl w-full max-h-[95vh] sm:max-h-[90vh] flex flex-col relative mx-2 sm:mx-0">
          ...
          {/* Close Button */}
          <button onClick={handleClose} className="absolute top-4 left-4 z-50 w-8 h-8 bg-white/80 hover:bg-white rounded-full shadow-lg flex items-center justify-center transition-all duration-200 hover:scale-110">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 sm:gap-8 relative z-10">
              ...
              <div className="flex items-center gap-2 sm:gap-4 mr-4 sm:mr-8 order-1 sm:order-2">
                <div className="flex flex-col items-center">
                  <div className={`relative w-12 h-12 rounded-xl flex items-center justify-center text-sm font-bold transition-all duration-500 ${step >= 1 ? 'bg-gradient-to-br from-emerald-500 to-teal-500 text-white shadow-lg scale-105' : 'bg-white/50 dark:bg-gray-800/50 text-gray-500 border border-gray-200/50'}`}>
                    <span className="relative z-10">1</span>
                  </div>
                  <span className="text-xs mt-2 text-gray-600 dark:text-gray-400 font-medium">البيانات</span>
                </div>

                <div className={`w-12 h-0.5 transition-all duration-500 ${step >= 2 ? 'bg-gradient-to-r from-emerald-500 to-teal-500' : 'bg-gray-200/50'}`}></div>

                <div className="flex flex-col items-center">
                  <div className={`relative w-12 h-12 rounded-xl flex items-center justify-center text-sm font-bold transition-all duration-500 ${step >= 2 ? 'bg-gradient-to-br from-emerald-500 to-teal-500 text-white shadow-lg scale-105' : 'bg-white/50 dark:bg-gray-800/50 text-gray-500 border border-gray-200/50'}`}>
          ...
          <div className="flex-1 overflow-y-auto px-4 sm:px-8 pb-4 sm:pb-8 mobile-smooth-scroll">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
                         {/* Name - 2/3 width */}
                          <div className="sm:col-span-2">
                           ...
                           <div className="relative">
                             <Input value={memberData.name} onChange={e => setMemberData({
                          ...memberData,
                          name: e.target.value
                        })} placeholder="أدخل الاسم الأول" className="h-9 text-sm border-2 border-emerald-200/50 dark:border-emerald-700/50 focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/20 transition-all duration-300 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-xl pr-10 font-arabic" />
                         ...
                         <div className="sm:col-span-1">
                          ...
                          <div className="sm:col-span-1">
                            ...
                            <div className="relative">
                              <Select value={memberData.gender} onValueChange={value => setMemberData({
                          ...memberData,
                          gender: value
                        })}>
                ...
                <div className="bg-white/40 dark:bg-gray-800/40 rounded-xl pb-6 px-6 border border-white/30 dark:border-gray-700/30">
                     ...
                     <div className="space-y-4">
                             ...
                             <div className="relative z-[10001]">
                               <Popover open={relationshipPopoverOpen} onOpenChange={setRelationshipPopoverOpen}>
                                 <PopoverTrigger asChild>
                                   <Button variant="outline" role="combobox" className="h-9 text-sm border-2 border-indigo-200/50 dark:border-indigo-700/50 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/20 transition-all duration-300 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-xl pr-10 font-arabic w-full justify-between">
                                      {memberData.selectedParent === null || memberData.selectedParent === "none" ? marriages.length > 0 ? "اختر علاقة القرابة مع العائلة" : "لا توجد زيجات متاحة" : marriages.find(m => m.id === memberData.selectedParent) ? (() => {
                              ...
                              if (husbandFromFamily && wifeFromFamily) {
                                ...
                                // الزوجة من العائلة
                                return (marriage.wife as any).is_founder ? `${marriage.wife.name} ${familyName} & ${marriage.husband.name}` : `${marriage.wife.name} ${(marriage.wife as any).father_name ? (marriage.wife as any).father_name + ' ' : ''}${familyName} & ${marriage.husband.name}`;
                              } else {
                                   ...
                                   <Command>
                                     ...
                                     <CommandList>
                                       <CommandEmpty className="font-arabic">لا توجد نتائج</CommandEmpty>
                                        <CommandGroup>
                                          {marriages.length === 0 && <CommandItem className="font-arabic text-gray-500">
                                              لا توجد زيجات متاحة - أضف أعضاء متزوجين أولاً
                                            </CommandItem>}
                                          {marriages.map(marriage => <CommandItem key={marriage.id} value={marriage.id} onSelect={value => {
                                  setMemberData({
                                    ...memberData,
                        ...
                        <div className="group">
                          ...
                          <div className="relative z-[10001]">
                            <Select value={memberData.isAlive ? "alive" : "deceased"} onValueChange={value => setMemberData({
                        ...memberData,
                        isAlive: value === "alive",
                        deathDate: value === "alive" ? null : memberData.deathDate
                      })}>
                         ...
                         {!memberData.isAlive && <div className="group">
                             <Label className="text-sm font-bold flex items-center gap-2 text-gray-700 dark:text-gray-300 mb-2 font-arabic">
                             ...
                             <div className="relative z-[10001]">
                               <EnhancedDatePicker value={memberData.deathDate} onChange={date => setMemberData({
                        ...memberData,
                        deathDate: date
                      })} placeholder="تاريخ الوفاة" className="h-9 text-sm border-2 border-gray-200/50 dark:border-gray-700/50 focus:border-gray-500 focus:ring-4 focus:ring-gray-500/20 transition-all duration-300 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-xl pr-10" />
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 items-start">
                    ...
                    <div className="group">
                      ...
                      <div className="border-2 border-dashed border-orange-200/50 dark:border-orange-700/50 rounded-xl p-4 bg-orange-50/30 dark:bg-orange-950/30 hover:border-orange-400 transition-all duration-300 h-[180px] flex flex-col">
                        <div className="flex flex-col items-center gap-3 flex-grow justify-center">
                          {memberData.croppedImage ? <div className="relative">
                              ...
                              {isImageUploadEnabled && <button onClick={() => setMemberData({
                          ...memberData,
                          image: null,
                          croppedImage: null
                            ...
                            </div> : <div className="w-20 h-20 bg-gradient-to-br from-orange-200 to-amber-200 dark:from-orange-800 to-amber-800 rounded-full flex items-center justify-center">
                              <Upload className="h-8 w-8 text-orange-600 dark:text-orange-300" />
                            </div>}
                           <div className="flex gap-2">
                            {!isImageUploadEnabled ? <div className="relative group">
                                <Button type="button" variant="outline" size="sm" disabled={true} className="gap-2 border-2 border-gray-300 text-gray-400 cursor-not-allowed opacity-60 transition-all duration-300 text-xs px-3 py-2">
                              ...
                              </div> : <Button type="button" variant="outline" size="sm" onClick={() => document.getElementById('member-image')?.click()} disabled={permissionLoading} className="gap-2 border-2 border-orange-300 text-orange-600 hover:bg-orange-50 dark:hover:bg-orange-950/50 transition-all duration-300 text-xs px-3 py-2">
                                <Upload className="h-3 w-3" />
                                {memberData.croppedImage ? 'تغيير' : 'اختيار'}
                              </Button>}
                          ...
                          <input id="member-image" type="file" accept="image/*" onChange={e => {
                        if (!isImageUploadEnabled) {
                          toast({
                            title: "ميزة غير متاحة",
                            description: "رفع الصور متاح فقط في الخطط المدفوعة. قم بترقية خطتك للاستفادة من هذه الميزة.",
                    ...
                    <div className="lg:col-span-3 group">
                      <Label className="text-sm font-bold flex items-center gap-2 text-gray-700 dark:text-gray-300 mb-2 font-arabic">
                       ...
                       <Textarea value={memberData.bio} onChange={e => setMemberData({
                    ...memberData,
                    bio: e.target.value
                  })} placeholder="أضف نبذة شخصية عن الفرد..." className="text-sm border-2 border-purple-200/50 dark:border-purple-700/50 focus:border-purple-500 focus:ring-4 focus:ring-purple-500/20 transition-all duration-300 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-xl resize-none h-[180px] font-arabic" />
                    </div>
                  </div>
                </div>
              </div>}
            ...
            {step === 2 && <div className="space-y-2">
                ...
                <div className="space-y-6">
                  {memberData.gender === "male" && <div className="bg-gradient-to-br from-pink-50 to-rose-100 dark:from-pink-950/30 dark:to-rose-900/30 rounded-2xl p-4 md:p-6 border border-pink-200/50 dark:border-pink-800/30 shadow-lg">
                       <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
                            ...
                            <div className="bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm border border-pink-200/50 dark:border-pink-800/30 rounded-xl p-6 shadow-md">
                              ...
                              <div className="space-y-4">
                                ...
                                <div>
                                  ...
                                  <div className="flex gap-3">
                                    <label className="flex items-center gap-2 cursor-pointer">
                                      <input type="radio" name="isFamilyMember" checked={newWife.isFamilyMember === true} onChange={() => {
                                setNewWife({
                                  ...newWife,
                                  isFamilyMember: true,
                                  existingFamilyMemberId: ''
                                ...
                                setCommandOpen(false);
                                    ...
                                    <label className="flex items-center gap-2 cursor-pointer">
                                      <input type="radio" name="isFamilyMember" checked={newWife.isFamilyMember === false} onChange={() => {
                                setNewWife({
                                  ...newWife,
                                  isFamilyMember: false,
                                  existingFamilyMemberId: ''
                                ...
                                {newWife.isFamilyMember && <div className="animate-fade-in">
                                    <Label className="text-sm font-bold flex items-center gap-2 text-gray-700 dark:text-gray-300 mb-2 font-arabic">
                                      <div className="w-2 h-2 bg-gradient-to-r from-purple-500 to-violet-500 rounded-full shadow-lg"></div>
                                    ...
                                    <div className="relative z-[10001]">
                                      <Popover open={commandOpen} onOpenChange={setCommandOpen}>
                                        <PopoverTrigger asChild>
                                          <Button variant="outline" role="combobox" aria-expanded={commandOpen} className="w-full justify-between h-9 text-sm border-2 border-purple-200/50 dark:border-purple-700/50 focus:border-purple-500 focus:ring-4 focus:ring-purple-500/20 transition-all duration-300 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-xl font-arabic">
                                            {newWife.existingFamilyMemberId ? familyMembers.find(m => m.id === newWife.existingFamilyMemberId)?.name : "اختر فرد من العائلة..."}
                                            <ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
                                          </Button>
                                        </PopoverTrigger>
                                        <PopoverContent className="w-full p-0 z-[10002]">
                                          <Command className="border-0 bg-white/95 dark:bg-gray-800/95 backdrop-blur-xl">
                                            ...
                                            <CommandList className="max-h-48">
                                              ...
                                              <CommandGroup>
                                                {familyMembers.filter(member => member.gender === 'female' && member.fatherId && familyMembers.some(fm => fm.id === member.fatherId)).map(member => <CommandItem key={member.id} value={`${member.name} ${familyMembers.find(f => f.id === member.fatherId)?.name || ''}`} onSelect={() => {
                                        setNewWife({
                                          ...newWife,
                                        ...
                                        setCommandOpen(false);
                                                      ...
                                                      <div className="flex items-center justify-between w-full">
                                                        ...
                                                        {member.fatherId && <span className="text-xs text-gray-500">
                                ...
                                {!newWife.isFamilyMember && <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                    {/* Name - 2/3 width */}
                                    <div className="sm:col-span-2">
                                      <Label className="text-sm font-bold flex items-center gap-2 text-gray-700 dark:text-gray-300 mb-2 font-arabic">
                                      ...
                                      <div className="relative">
                                        <Input value={newWife.name} onChange={e => setNewWife({
                                ...newWife,
                                name: e.target.value
                              })} className="h-9 text-sm border-2 border-pink-200/50 dark:border-pink-700/50 focus:border-pink-500 focus:ring-4 focus:ring-pink-500/20 transition-all duration-300 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-xl pr-10 font-arabic" placeholder="اسم الزوجة" disabled={newWife.isFamilyMember && !!newWife.existingFamilyMemberId} />
                                    ...
                                    <div className="sm:col-span-1">
                                      ...
                                      <div className="relative z-[10001]">
                                        <EnhancedDatePicker value={newWife.birthDate ? new Date(newWife.birthDate) : null} onChange={date => setNewWife({
                                ...newWife,
                                birthDate: date ? date.toISOString().split('T')[0] : ''
                              })} placeholder="اختر التاريخ" className="h-9 text-sm border-2 border-rose-200/50 dark:border-rose-700/50 focus:border-rose-500 focus:ring-4 focus:ring-rose-500/20 transition-all duration-300 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-xl pr-10 font-arabic" disabled={newWife.isFamilyMember && !!newWife.existingFamilyMemberId} />
                                ...
                                <div>
                                  ...
                                  <div className="relative z-[10001]">
                                    <Select value={newWife.maritalStatus || "married"} onValueChange={value => setNewWife({
                              ...newWife,
                              maritalStatus: value
                            })}>
                                ...
                                {!newWife.isFamilyMember && <div className="space-y-4">
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                      ...
                                      <div>
                                        ...
                                        <div className="relative z-[10001]">
                                          <Select value={newWife.isAlive ? "alive" : "deceased"} onValueChange={value => setNewWife({
                                  ...newWife,
                                  isAlive: value === "alive",
                                  deathDate: value === "alive" ? "" : newWife.deathDate
                                })}>
                                      ...
                                      {!newWife.isAlive && <div className="animate-fade-in">
                                          <Label className="text-sm font-bold flex items-center gap-2 text-gray-700 dark:text-gray-300 mb-2 font-arabic">
                                          ...
                                          <div className="relative z-[10000]">
                                            <EnhancedDatePicker value={newWife.deathDate ? new Date(newWife.deathDate) : null} onChange={date => setNewWife({
                                  ...newWife,
                                  deathDate: date ? date.toISOString().split('T')[0] : ''
                                })} placeholder="اختر تاريخ الوفاة" className="h-9 text-sm border-2 border-red-200/50 dark:border-red-700/50 focus:border-red-500 focus:ring-4 focus:ring-red-500/20 transition-all duration-300 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-xl pr-10 font-arabic" />
                                ...
                                {!newWife.isFamilyMember && <div>
                                    <label className="block text-sm font-medium text-pink-700 dark:text-pink-300 mb-2">
                                      الصورة الشخصية
                                    </label>
                                    {isImageUploadEnabled ? <>
                                        <input type="file" accept="image/*" onChange={e => handleNewWifeImageSelect(e)} className="hidden" id="new-wife-image" />
                                        
                                        {newWife.imageUrl ? <div className="flex items-center space-x-3 p-3 bg-pink-50/50 dark:bg-pink-950/30 rounded-lg border border-pink-200/30 dark:border-pink-800/30">
                                            <img src={newWife.imageUrl} alt="New Wife" className="w-12 h-12 rounded-full object-cover border-2 border-pink-300" />
                                            <div className="flex flex-col space-y-1">
                                              <Button type="button" variant="outline" size="sm" onClick={() => document.getElementById('new-wife-image')?.click()} className="h-6 text-xs px-2 border-pink-300 text-pink-600 hover:bg-pink-50">
                                                <Camera className="w-2 h-2 mr-1" />
                                                تغيير
                                              </Button>
                                              <Button type="button" variant="outline" size="sm" onClick={() => setNewWife({
                                  ...newWife,
                                  imageUrl: ''
                                          ...
                                          </div> : <label htmlFor="new-wife-image" className="flex flex-col items-center justify-center w-full h-20 border-2 border-dashed border-pink-300 dark:border-pink-700 cursor-pointer hover:border-pink-400 hover:bg-pink-50/30 dark:hover:bg-pink-900/20 rounded-xl transition-all duration-200 group">
                          ...
                          addWife();
                        }} className="w-full bg-gradient-to-r from-pink-500 to-rose-500 hover:from-pink-600 hover:to-rose-600 text-white shadow-md" size="lg" disabled={!newWife.name.trim()}>
                          ...
                          <div className="w-full">
                            <div className="flex items-center gap-2 mb-6">
                              ...
                              {wives.length > 0 && <span className="px-2 py-1 bg-rose-100 dark:bg-rose-900/30 text-rose-600 dark:text-rose-400 text-xs rounded-full font-medium">
                                  {wives.length}
                                </span>}
                            </div>
                            
                            <div className="space-y-4 max-h-96 overflow-y-auto pr-2">
                              {wives.length === 0 ? <div className="text-center py-12 bg-white/50 dark:bg-gray-900/50 rounded-xl border border-pink-200/50 dark:border-pink-800/30">
                                ...
                                </div> : wives.map((wife, index) => <div key={index} className="bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm border border-pink-200/50 dark:border-pink-800/30 rounded-xl p-4 shadow-md hover:shadow-lg transition-all duration-200 group">
                                    <div className="flex items-start justify-between">
                                      <div className="flex items-center space-x-4">
                                        {wife.croppedImage ? <div className="relative">
                                            <img src={wife.croppedImage} alt={wife.name} className="w-14 h-14 rounded-full object-cover border-2 border-pink-300 shadow-sm" />
                                          ...
                                          </div> : <div className="w-14 h-14 rounded-full bg-gradient-to-br from-pink-100 to-rose-100 dark:from-pink-900/50 dark:to-rose-900/50 flex items-center justify-center border-2 border-pink-300 dark:border-pink-700">
                                        ...
                                        <div className="flex-1">
                                          ...
                                          <div className="flex flex-wrap items-center gap-2 text-sm text-gray-600 dark:text-gray-400 mt-1">
                                            {wife.birthDate && <span className="px-2 py-1 bg-pink-50 dark:bg-pink-900/30 text-pink-600 dark:text-pink-400 rounded-full text-xs">
                                                المولد: {new Date(wife.birthDate).toLocaleDateString('ar')}
                                              </span>}
                                            {!wife.isAlive && wife.deathDate && <span className="px-2 py-1 bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 rounded-full text-xs">
                                      ...
                                      <div className="flex space-x-reverse space-x-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                                        <Button type="button" variant="outline" size="sm" onClick={() => editWife(index)} className="border-pink-300 text-pink-600 hover:bg-pink-50 hover:border-pink-400 bg-pink-50/50 dark:bg-pink-950/20">
                                          <Edit3 className="w-4 h-4" />
                                        </Button>
                                        <Button type="button" variant="outline" size="sm" onClick={() => removeWife(index)} className="border-red-300 text-red-600 hover:bg-red-50 hover:border-red-400 bg-red-50/50 dark:bg-red-950/20">
                    ...
                    {memberData.gender === "female" && <div className="bg-gradient-to-br from-sky-50 to-blue-100 dark:from-sky-950/30 dark:to-blue-900/30 rounded-2xl p-4 md:p-6 border border-sky-200/50 dark:border-sky-800/30 shadow-lg">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-8">
                          ...
                          <div className="w-full md:col-span-2">
                             <div className="bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm border border-sky-200/50 dark:border-sky-800/30 rounded-xl p-6 shadow-md">
                               <div className="flex items-center gap-2 mb-6">
                                ...
                                <div className="space-y-4">
                                  {/* Question about existing husband */}
                                  <div className="mb-6">
                                    <Label className="text-sm font-bold flex items-center gap-2 text-gray-700 dark:text-gray-300 mb-3 font-arabic">
                                      ...
                                      هل يوجد زوج في العائلة؟
                                    </Label>
                                    <div className="flex gap-4">
                                      <label className="flex items-center gap-2 cursor-pointer">
                                        <input type="radio" name="husbandExists" value="existing" checked={newHusband.isFamilyMember} onChange={() => setNewHusband({
                                ...newHusband,
                                isFamilyMember: true,
                                existingFamilyMemberId: "",
                                name: "",
                                birthDate: ""
                                        ...
                                        <span className="text-sm font-arabic">نعم، موجود في العائلة</span>
                                      </label>
                                      <label className="flex items-center gap-2 cursor-pointer">
                                        <input type="radio" name="husbandExists" value="new" checked={!newHusband.isFamilyMember} onChange={() => setNewHusband({
                                ...newHusband,
                                isFamilyMember: false,
                                existingFamilyMemberId: ""
                              })} className="text-sky-500 focus:ring-sky-500" />
                                        <span className="text-sm font-arabic">لا، زوج جديد</span>
                                      </label>
                                    </div>
                                  </div>

                                  {/* Existing husband selection */}
                                  {newHusband.isFamilyMember && <div className="space-y-4">
                                      <div>
                                        <Label className="text-sm font-bold flex items-center gap-2 text-gray-700 dark:text-gray-300 mb-2 font-arabic">
                                          <div className="w-2 h-2 bg-gradient-to-r from-sky-500 to-blue-500 rounded-full shadow-lg"></div>
                                          اختر الزوج *
                                        ...
                                        <div className="relative z-[10001]">
                                          <Select value={newHusband.existingFamilyMemberId} onValueChange={value => {
                                ...
                                setNewHusband({
                                  ...
                                  name: selectedMember?.name || ""
                                            ...
                                            <SelectTrigger className="h-9 text-sm border-2 border-sky-200/50 dark:border-sky-700/50 focus:border-sky-500 focus:ring-4 focus:ring-sky-500/20 transition-all duration-300 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-xl pr-10 font-arabic">
                                              <SelectValue placeholder="اختر الزوج من العائلة" />
                                            </SelectTrigger>
                                            <SelectContent className="bg-card/95 backdrop-blur-xl border-border/50 z-[10002]">
                                              {familyMembers.filter(member => member.gender === "male" && member.id !== memberData.selectedParent).map(member => {
                                    const fatherName = member.fatherId ? familyMembers.find(f => f.id === member.fatherId)?.name : null;
                                    return <SelectItem key={member.id} value={member.id} className="font-arabic text-sm">
                                                    {member.name} {fatherName && `(ابن ${fatherName})`}
                                                  </SelectItem>;
                                  })}
                                            </SelectContent>
                                          </Select>
                                      ...
                                      {/* Marital Status for existing husband */}
                                      <div>
                                        <Label className="text-sm font-bold flex items-center gap-2 text-gray-700 dark:text-gray-300 mb-2 font-arabic">
                                        ...
                                        <div className="relative z-[10001]">
                                          <Select value={newHusband.maritalStatus || "married"} onValueChange={value => setNewHusband({
                                ...newHusband,
                                maritalStatus: value
                              })}>
                                  ...
                                  {!newHusband.isFamilyMember && <div className="space-y-4">
                                      ...
                                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                        ...
                                        <div className="sm:col-span-2">
                                          ...
                                          <div className="relative">
                                            <Input value={newHusband.name} onChange={e => setNewHusband({
                                  ...newHusband,
                                  name: e.target.value
                                })} className="h-9 text-sm border-2 border-sky-200/50 dark:border-sky-700/50 focus:border-sky-500 focus:ring-4 focus:ring-sky-500/20 transition-all duration-300 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-xl pr-10 font-arabic" placeholder="اسم الزوج" />
                                            <div className="absolute right-3 top-1/2 transform -translate-y-1/2 w-4 h-4 bg-gradient-to-br from-sky-500 to-blue-500 rounded-lg flex items-center justify-center">
                                              <Heart className="h-2 w-2 text-white" />
                                            </div>
                                          </div>
                                        ...
                                        <div className="sm:col-span-1">
                                          ...
                                          <div className="relative z-[10001]">
                                            <EnhancedDatePicker value={newHusband.birthDate ? new Date(newHusband.birthDate) : null} onChange={date => setNewHusband({
                                  ...newHusband,
                                  birthDate: date ? date.toISOString().split('T')[0] : ''
                                })} placeholder="اختر التاريخ" className="h-9 text-sm border-2 border-blue-200/50 dark:border-blue-700/50 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/20 transition-all duration-300 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-xl pr-10 font-arabic" />
                                    ...
                                    maritalStatus: value
                                          ...
                                          <div>
                                            ...
                                            <div className="relative z-[10001]">
                                              <Select value={newHusband.isAlive ? "alive" : "deceased"} onValueChange={value => setNewHusband({
                                    ...newHusband,
                                    isAlive: value === "alive",
                                    deathDate: value === "alive" ? "" : newHusband.deathDate
                                  })}>
                                                <SelectTrigger className="h-9 text-sm border-2 border-emerald-200/50 dark:border-emerald-700/50 focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/20 transition-all duration-300 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-xl pr-10 font-arabic">
                                                  <SelectValue placeholder="الحالة" />
                                                </SelectTrigger>
                                          ...
                                          {/* Death Date - only show if deceased */}
                                          {!newHusband.isAlive && <div className="animate-fade-in">
                                              <Label className="text-sm font-bold flex items-center gap-2 text-gray-700 dark:text-gray-300 mb-2 font-arabic">
                                                <div className="w-2 h-2 bg-gradient-to-r from-red-500 to-rose-500 rounded-full shadow-lg group-hover:scale-110 transition-transform"></div>
                                              تاريخ الوفاة
                                              </Label>
                                              <div className="relative z-[10000]">
                                                <EnhancedDatePicker value={newHusband.deathDate ? new Date(newHusband.deathDate) : null} onChange={date => setNewHusband({
                                    ...newHusband,
                                    deathDate: date ? date.toISOString().split('T')[0] : ''
                                  })} placeholder="اختر تاريخ الوفاة" className="h-9 text-sm border-2 border-red-200/50 dark:border-red-700/50 focus:border-red-500 focus:ring-4 focus:ring-red-500/20 transition-all duration-300 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-xl pr-10 font-arabic" />
                                                <div className="absolute right-3 top-1/2 transform -translate-y-1/2 w-4 h-4 bg-gradient-to-br from-red-500 to-rose-500 rounded-lg flex items-center justify-center">
                                ...
                                imageUrl: ''
                                        ...
                                        <Tooltip>
                                          <TooltipTrigger asChild>
                                            <div>
                                              <label htmlFor={isImageUploadEnabled ? "new-husband-image" : undefined} className={cn("flex flex-col items-center justify-center w-full h-24 border-2 border-dashed rounded-xl transition-all duration-200 group", isImageUploadEnabled ? "border-sky-300 dark:border-sky-700 cursor-pointer hover:border-sky-400 hover:bg-sky-50/30 dark:hover:bg-sky-900/20" : "border-gray-300 dark:border-gray-600 cursor-not-allowed opacity-60")}>
                                                <Camera className={cn("w-6 h-6 mb-1", isImageUploadEnabled ? "text-sky-400 group-hover:text-sky-500" : "text-gray-400")} />
                                                <span className={cn("text-sm text-center", isImageUploadEnabled ? "text-sky-600 dark:text-sky-400" : "text-gray-400")}>إضافة صورة</span>
                                              </label>
                                            </div>
                                          </TooltipTrigger>
                                          {!isImageUploadEnabled && <TooltipContent side="top" className="bg-gradient-to-r from-amber-600 to-orange-600 text-white border-0 shadow-lg">
                          ...
                          addHusband();
                        }} className="w-full bg-gradient-to-r from-sky-500 to-blue-500 hover:from-sky-600 hover:to-blue-600 text-white shadow-md" size="lg" disabled={!newWife.name.trim()}>
                                   <Plus className="w-4 h-4 mr-2" />
                                   إضافة الزوج
                                 </Button>
                               </div>
                          ...
                          <div className="w-full md:col-span-1">
                            <div className="flex items-center gap-2 mb-6">
                              ...
                              {wives.length > 0 && <span className="px-2 py-1 bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 text-xs rounded-full font-medium">
                                  {wives.length}
                                </span>}
                            </div>
                                  ...
                                  <div className="flex items-start justify-between">
                                     <div className="flex items-center space-x-4">
                                       {husband.croppedImage ? <div className="relative">
                                           <img src={husband.croppedImage} alt={husband.name} className="w-14 h-14 rounded-full object-cover border-2 border-sky-300 shadow-sm" />
                                         ...
                                         </div> : <div className="w-14 h-14 rounded-full bg-gradient-to-br from-sky-100 to-blue-100 dark:from-sky-900/50 dark:to-blue-900/50 flex items-center justify-center border-2 border-sky-300 dark:border-sky-700">
                                           <User className="w-7 h-7 text-sky-500" />
                                         </div>}
                                       <div className="flex-1">
                                         <h5 className="font-semibold text-gray-800 dark:text-gray-200 text-lg">{husband.name}</h5>
                                         <div className="flex flex-wrap items-center gap-2 text-sm text-gray-600 dark:text-gray-400 mt-1">
                                           {husband.birthDate && <span className="px-2 py-1 bg-sky-50 dark:bg-sky-900/30 text-sky-600 dark:text-sky-400 rounded-full text-xs">
                                               المولد: {new Date(husband.birthDate).toLocaleDateString('ar')}
                                             </span>}
                                           {!husband.isAlive && husband.deathDate && <span className="px-2 py-1 bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 rounded-full text-xs">
          ...
          <div className="flex-shrink-0 border-t border-white/40 dark:border-gray-600/40 p-3 sm:p-2 bg-white/20 dark:bg-gray-800/20 backdrop-blur-xl mt-4">{/* moved up with mt-4 */}
            <div className="flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-4">
              {step > 1 && <Button type="button" onClick={() => setStep(step - 1)} variant="outline" className="gap-2 px-6 py-3 rounded-xl border-2 hover:bg-gray-50 dark:hover:bg-gray-800">
              ...
              setStep(2);
            }} disabled={!memberData.name.trim() || !memberData.gender} className="gap-2 bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white px-8 py-3 rounded-xl shadow-lg transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:from-emerald-500 disabled:hover:to-teal-500">
                  التالي
              ...
              handleSubmit();
            }} disabled={isSubmitting} className="gap-2 bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white px-8 py-3 rounded-xl shadow-lg transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed">
      ...
      {/* Crop Modal */}
      {showCropModal && cropImage && <div className="fixed inset-0 z-[9999] bg-black/80 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-gray-900 rounded-xl shadow-2xl max-w-md w-full p-6">
            
            
            <div className="relative w-full h-64 mb-4">
              <Cropper image={cropImage} crop={crop} zoom={zoom} aspect={1} onCropChange={setCrop} onZoomChange={setZoom} onCropComplete={(croppedArea, croppedAreaPixels) => {
